...
void main(){
    ...
    float topBoxAmpl=20.;
    vec2 topBoxPos=st-vec2(.52,.89)+noise(st*topBoxFreq)/topBoxAmpl;
    vec2 topBoxDimensions=vec2(.8,.005);
    ...
    vec2 midBoxDimensions=vec2(1.,.14);
    ...
    vec2 botBoxDimensions=vec2(1.,.005);
    float botBox=step(.1,sdBox(botBoxPos,botBoxDimensions));
    color=mix(color,vec3(0.3333, 0.4431, 0.4588),1.-botBox);
    
    // make a frame
    color=mix(color,vec3(0.),step(0.5,st.x)+step(1.5,1.-st.x));
