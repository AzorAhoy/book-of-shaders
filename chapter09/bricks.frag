...
vec2 movingBrickTiles(vec2 _st, float _zoom, float _speed){
    _st *= _zoom;
...
vec2 movingBrickTiles2(vec2 _st, float _zoom, float _speed){
    _st *= _zoom;
    float time = u_time*_speed;
    if( fract(time)>0.5 ){
        if (fract( _st.y * 0.5) > 0.5){
...
void main(void){
    ...
    // st = brickTile(st,5.0);
    // st = movingBrickTiles(st,5.0, 0.3);
    st = movingBrickTiles2(st,5.0, 0.3);
